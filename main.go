...
const (
	defaultPMUServerAddress           = "localhost:8008"
	defaultSynchrophasorServerAddress = "localhost:9009"
...
func main() {
	...
	for {
		...
		pmuClient, pmuConn, err := pmuConnect()
		if err != nil {
			glog.Errorf("Error connecting to PMU server: %v", err)
		}

		dpeClient, dpeConn, err := dpeConnect()
		if err != nil {
...
func pmuConnect() (pmu_server.SynchrophasorDataClient, *grpc.ClientConn, error) {
...
func dpeConnect() (synchrophasor_dpe.SynchrophasorDPEClient, *grpc.ClientConn, error) {
	conn, err := grpc.Dial(defaultSynchrophasorServerAddress, grpc.WithInsecure(), grpc.WithTimeout(time.Duration(20)*time.Second))
	if err != nil {
		return nil, nil, err
	}
